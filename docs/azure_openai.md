...
Azure OpenAI support lets Zen MCP talk to GPT-4o, GPT-4.1, gpt-5.6-terra, and o-series deployments that you expose through your Azure resource. This guide describes the configuration expected by the server: a couple of required environment variables plus a JSON manifest that lists every deployment you want to expose.
